...
-- Move pg_trgm to extensions schema for security best practice
DROP EXTENSION IF EXISTS pg_trgm;
CREATE EXTENSION IF NOT EXISTS pg_trgm SCHEMA extensions;

-- Recreate the function using extensions.similarity
...
BEGIN
  ...
  SELECT 
    ...
    f.id AS duplicate_forecast_id,
    f.title AS duplicate_title,
    extensions.similarity(lower(p_title), lower(f.title))::numeric AS similarity_score
  FROM forecasts f
  WHERE f.project_a_id = p_project_a_id
    AND f.end_date > now()
    AND f.status = 'active'
    AND extensions.similarity(lower(p_title), lower(f.title)) >= 0.5
  ORDER BY extensions.similarity(lower(p_title), lower(f.title)) DESC
  LIMIT 1;
