 ...
 */
export function getWeightedChance(forecast: {
  weighted_votes_yes?: number;
  weighted_votes_no?: number;
  total_votes_yes?: number;
  total_votes_no?: number;
}): { yesPct: number; noPct: number } {
  const wy = Number((forecast as any).weighted_votes_yes) || 0;
  const wn = Number((forecast as any).weighted_votes_no) || 0;
  const total = wy + wn;
  if (total > 0) {
  ...
  const ty = forecast.total_votes_yes ?? 0;
  const tn = forecast.total_votes_no ?? 0;
  const t2 = ty + tn;
  if (t2 > 0) {
