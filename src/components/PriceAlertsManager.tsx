...
import { Bell, BellOff, Trash2, Plus, ChevronUp, ChevronDown, TrendingUp, TrendingDown, ArrowUpDown, X } from "lucide-react";
...
export default function PriceAlertsManager({ projects, holdingProjectIds }: PriceAlertsManagerProps) {
  ...
  // Projects the user holds but doesn't have alerts for
    ...
    <div className="rounded-xl border border-border bg-card overflow-hidden">
      <div className="flex items-center justify-between p-4 md:p-5 border-b border-border">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-[11px] text-muted-foreground">{alerts.filter((a) => a.is_enabled).length} active</p>
          </div>
        </div>
        <Button
          ...
          >
            <div className="p-4 md:p-5 border-b border-border bg-secondary/20">
              <div className="flex flex-col sm:flex-row sm:items-end gap-3">
                ...
                <Button onClick={handleAdd} disabled={upsertAlert.isPending} className="h-9 gap-1.5 w-full sm:w-auto">
      ...
      {isLoading ? (
        ...
        <div className="p-8 md:p-12 text-center">
          <div className="mx-auto h-12 w-12 rounded-2xl bg-secondary/50 flex items-center justify-center mb-3">
          ...
          {alerts.map((alert, idx) => {
            ...
            return (
              <motion.div
                ...
                className={`flex items-center justify-between p-3.5 md:p-4 transition-colors hover:bg-secondary/10 ${!alert.is_enabled ? "opacity-50" : ""}`}
              >
                <div className="flex items-center gap-3 min-w-0 flex-1">
                  ...
                  <div className="min-w-0">
                    <p className="text-sm font-medium text-foreground truncate">
                      {project?.name || "Unknown"} <span className="text-muted-foreground">({project?.token})</span>
                    </p>
