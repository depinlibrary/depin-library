...
export default function PredictionHeader({ prediction, yesPct, totalVotes, isEnded, timeLeft }: PredictionHeaderProps) {
  const handleShareX = () => {
    ...
    const text = `${prediction.title} — ${yesPct.toFixed(0)}% Yes | ${totalVotes} votes`;
        ...
        <div className="flex items-center gap-3 mb-5">
          <div className="flex items-center -space-x-2">
            {prediction.project_a?.logo_url ? (
              <img src={prediction.project_a.logo_url} alt={prediction.project_a.name} className="w-10 h-10 rounded-[7px] object-contain border-2 border-card bg-secondary relative z-10" />
            ) : (
              <span className="w-10 h-10 rounded-[7px] flex items-center justify-center text-lg border-2 border-card bg-secondary relative z-10">{prediction.project_a?.logo_emoji || "⬡"}</span>
            )}
            {prediction.project_b && (
              prediction.project_b.logo_url ? (
                <img src={prediction.project_b.logo_url} alt={prediction.project_b.name} className="w-10 h-10 rounded-[7px] object-contain border-2 border-card bg-secondary" />
              ) : (
                <span className="w-10 h-10 rounded-[7px] flex items-center justify-center text-lg border-2 border-card bg-secondary">{prediction.project_b?.logo_emoji || "⬡"}</span>
              )
            )}
          </div>
          <div className="flex items-center gap-2 flex-wrap">
            <Link to={`/project/${prediction.project_a?.slug}`} className="text-xs font-semibold text-foreground/80 hover:text-primary transition-colors">
              {prediction.project_a?.name}
            </Link>
            {prediction.project_b && (
              <>
                <span className="text-muted-foreground/40 text-[10px] font-medium uppercase tracking-widest">vs</span>
                <Link to={`/project/${prediction.project_b?.slug}`} className="text-xs font-semibold text-foreground/80 hover:text-primary transition-colors">
                  {prediction.project_b?.name}
                </Link>
              </>
            )}
          </div>
          ...
          <div className="flex items-center gap-4 text-[11px] text-muted-foreground">
            <UserStatsHoverCard userId={prediction.creator_user_id} displayName={prediction.creator_name} avatarUrl={prediction.creator_avatar_url}>
              <span className="flex items-center gap-1.5 cursor-pointer hover:text-foreground transition-colors">
                <UserAvatar avatarUrl={prediction.creator_avatar_url} displayName={prediction.creator_name} size="sm" />
                {prediction.creator_name}
              </span>
            </UserStatsHoverCard>
            <span className="hidden sm:flex items-center gap-1"><CalendarDays className="h-3 w-3" /> {format(new Date(prediction.created_at), "MMM d, yyyy")}</span>
            <span className="flex items-center gap-1"><Timer className="h-3 w-3" /> Ends {format(new Date(prediction.end_date), "MMM d, yyyy")}</span>
          </div>
