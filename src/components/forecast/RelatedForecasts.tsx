  ...
  return (
    ...
    >
      <h2 className="text-sm font-semibold text-foreground mb-4 flex items-center gap-2">
        <Zap className="h-4 w-4 text-primary" /> Related Forecasts
      </h2>
        ...
        {forecasts.map((rf: any) => {
          ...
          return (
            <Link
              key={rf.id}
              to={`/forecasts/${rf.id}`}
              className="rounded-xl border border-border bg-card p-4 hover:border-primary/30 hover:shadow-lg hover:shadow-primary/5 transition-all group"
            >
              ...
              <h3 className="text-xs font-semibold text-foreground leading-snug line-clamp-2 mb-2.5 group-hover:text-primary transition-colors">
                {rf.title}
              </h3>
