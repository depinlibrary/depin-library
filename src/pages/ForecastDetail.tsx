...
import { ArrowLeft, Clock, CalendarDays, Timer, Users, ExternalLink, Copy, ArrowUpRight, ArrowDownRight, ThumbsUp, ThumbsDown, Gauge, Target, Zap, CheckCircle2, XCircle, MessageSquare, BarChart3 } from "lucide-react";
...
const ForecastDetail = () => {
  ...
  useEffect(() => {
    ...
    if (isEnded) { setCountdown("Ended"); return; }
    const tick = () => {
      const diff = new Date(forecast.end_date).getTime() - Date.now();
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <main className="container mx-auto px-4 pt-20 pb-16 max-w-6xl">
        ...
        {/* ═══════ TOP HERO: Title + Status + Outcomes ═══════ */}
        <motion.div
          ...
          <div className="p-6 sm:p-8">
            ...
            <div className="flex items-center justify-between mb-5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-1.5 mt-0.5">
                    ...
                    <span className={`text-[10px] font-semibold ${isEnded ? 'text-destructive' : 'text-green-600 dark:text-green-400'}`}>
              ...
              <div className="flex items-center gap-1.5">
                {!isEnded && countdown !== "Ended" && (
                  <span className="text-xs font-bold font-['Space_Grotesk'] tabular-nums text-primary mr-2">
                    {countdown}
                  </span>
                )}
            ...
            {forecast.status === "resolved" && isPriceMarket && forecast.prediction_target != null && (
              <motion.div
                ...
                className="mb-5 rounded-xl border border-primary/30 bg-primary/5 px-4 py-3 flex items-center gap-3 relative overflow-hidden"
              >
                ...
                <motion.div className="flex items-center justify-center h-8 w-8 rounded-full bg-primary/15 shrink-0" animate={{ scale: [1, 1.1, 1] }} transition={{ duration: 2, repeat: Infinity, ease: "easeInOut" }}>
                ...
                <Badge variant="outline" className="border-primary/40 text-primary text-[10px] shrink-0 relative">Early Close</Badge>
              </motion.div>
            )}

            {/* Outcome rows — Polymarket clean style */}
            <div className="space-y-1 mb-5">
              <div className="flex items-center justify-between py-3 px-3 rounded-xl hover:bg-secondary/30 transition-colors">
            ...
            {/* Result + Your Prediction if ended */}
            {isEnded && (() => {
              const outcomeResult = forecast.outcome ? forecast.outcome : (yesPct >= 50 ? "yes" : "no");
              ...
              return (
                <div className="space-y-2 mb-5">
                  <div className="flex items-center justify-end">
                    <Badge variant="secondary" className={`text-[10px] font-semibold ${outcomeIsLong ? "bg-green-500/10 text-green-600 dark:text-green-400 border-green-500/20" : "bg-destructive/10 text-destructive border-destructive/20"}`}>
                  ...
                  {userVote && (
                    <div className={`flex items-center justify-between rounded-lg px-4 py-2.5 text-xs font-semibold ${
                      ...
                      <span className="flex items-center gap-1">
                        {userCorrect ? <><CheckCircle2 className="h-3.5 w-3.5" /> Correct</> : <><XCircle className="h-3.5 w-3.5" /> Incorrect</>}
                      </span>
                    </div>
                  )}
                </div>
            ...
            <div className="grid grid-cols-4 gap-3 pt-4 border-t border-border">
              ...
              <div className="text-center">
                <p className="text-[10px] text-muted-foreground mb-0.5">{yesLabel}</p>
                <p className="text-sm font-bold text-primary font-['Space_Grotesk']">{forecast.total_votes_yes}</p>
              </div>
              <div className="text-center">
                <p className="text-[10px] text-muted-foreground mb-0.5">{noLabel}</p>
        ...
        {/* ═══════ TWO COLUMN LAYOUT ═══════ */}
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
          {/* ═══════ LEFT COLUMN (3/5): Charts + Tabs ═══════ */}
          <div className="lg:col-span-3 space-y-5">
            {/* Vote / Confidence — inline for active forecasts */}
            {!isEnded && (
              ...
              >
                <div className="p-5">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    {forecast.user_vote && (
                      <span className="text-[10px] text-muted-foreground">
                        Voted <span className={`font-semibold ${forecast.user_vote === "yes" ? "text-primary" : "text-destructive"}`}>{forecast.user_vote === "yes" ? yesLabel : noLabel}</span>
                      </span>
                    )}
                  </div>
                  {/* Confidence slider */}
                  <div className="rounded-xl bg-secondary/30 border border-border/50 px-4 py-3 mb-4">
                    <div className="flex items-center justify-between mb-2.5">
                      <div className="flex items-center gap-1.5">
                        <Gauge className="h-3.5 w-3.5 text-muted-foreground" />
                        <span className="text-[11px] font-medium text-muted-foreground">Confidence</span>
                      </div>
                      <span className={`text-[11px] font-semibold ${confInfo.color}`}>{confInfo.label} ({confidence}/5)</span>
                    </div>
                  ...
                  <div className="flex gap-2.5">
                    <button onClick={() => handleVote("yes")} className={`flex-1 rounded-lg py-3 text-sm font-bold transition-all duration-200 ${
                      forecast.user_vote === "yes" ? "bg-primary text-primary-foreground" : "bg-primary/10 text-primary hover:bg-primary/20"
                    }`}>
                      {forecast.user_vote === "yes" ? `Voted ${yesLabel} ✓` : yesLabel}
                    </button>
                    <button onClick={() => handleVote("no")} className={`flex-1 rounded-lg py-3 text-sm font-bold transition-all duration-200 ${
                      forecast.user_vote === "no" ? "bg-destructive text-destructive-foreground" : "bg-destructive/10 text-destructive hover:bg-destructive/20"
                    }`}>
                      {forecast.user_vote === "no" ? `Voted ${noLabel} ✓` : noLabel}
                    </button>
                  </div>
                  {forecast.user_vote && (
                    <motion.p initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} className="text-[10px] text-muted-foreground text-center mt-3">
                      Vote again to change your prediction
                    </motion.p>
                  )}
                </div>
              </motion.div>
            ...
            {isEnded && !isPriceMarket && (
              <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} className="rounded-2xl border border-border bg-card overflow-hidden">
                <div className="p-5">
          ...
          {/* ═══════ RIGHT COLUMN (2/5): Creator + Analysis + Related ═══════ */}
          <div className="lg:col-span-2 space-y-5">
            ...
            {/* Vote Trend — for token_price/market_cap */}
            {(forecastDimension === "token_price" || forecastDimension === "market_cap") && (
              <VoteHistoryChart voteHistory={voteHistory} yesLabel={yesLabel} noLabel={noLabel} />
            )}
